...
> `-c constraints.txt` 를 빼면 numpy 가 2.x 로 올라가 mediapipe 가 동작하지 않습니다.
> SMPL-X 모델은 [직접 다운로드](https://smpl-x.is.tue.mpg.de) 후 `assets/models/smplx/` 에 배치하세요.

API 명세 · 측정 부위 이름 · 파일명 규칙은 [docs/CONVENTIONS.md](docs/CONVENTIONS.md) 를 참고하세요.
...
- 리뷰·테스트를 거쳐 main 으로 PR 하고, 그 머지가 배포를 일으킵니다
