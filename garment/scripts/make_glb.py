...
def colors(d):
    ...
    tight = d < 0.9
    t = np.clip((0.9 - d[tight]) / 0.7, 0, 1)[:, None]
    c[tight] = GREEN + (RED - GREEN) * t
    loose = d > 2.0
    t = np.clip((d[loose] - 2.0) / 2.0, 0, 1)[:, None]
...
for obj in sorted(Path('Sim_results').glob('*.obj')):
    ...
    for _ in range(1):
        d = np.array([d[i] if not nbr[i] else (d[i] + d[nbr[i]].sum()) / (1 + len(nbr[i]))
    ...
    log[name] = {'fit': SPEC[key]['fit'],
                 ...
                 'loose_pct': round(float((d > 2.0).mean()*100), 1),
