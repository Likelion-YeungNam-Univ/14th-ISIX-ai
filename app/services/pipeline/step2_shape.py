...
# 고정 A자세. 의류 파트의 GarmentCode 패턴과 팔 각도를 맞춘다.
#
# 주의 — 아래 두 값은 "회전량"이지 결과 각도가 아니다.
# SMPL-X 는 T포즈 팔이 정확히 수평이 아니고 쇄골 관절도 함께 돌아서,
# 회전량과 실제 각도의 합이 90 이 아니라 약 78.7 이다.
# 계약값은 반드시 아래 "결과" 각도로 주고받을 것. 세 번 어긋났던 지점이다.
#
#   회전량 (어깨 33.189 / 팔꿈치 13.237)
#     -> 결과: 상완 45.483도, 전완 45.483도  (수직 아래 = 0도 기준)
#
# 팔꿈치를 펴는 이유: 긴팔 소매가 팔꿈치를 지나므로, 상완만 맞추고 전완이
# 굽어 있으면 팔꿈치 아래에서 옷이 몸을 뚫는다.
SHOULDER_DOWN_DEG = 33.189
ELBOW_STRAIGHTEN_DEG = 13.237

# 계약값 (측정 결과). body_grid.json 에 이 값을 싣는다.
ARM_POSE_ANGLE_DEG = 45.483      # 상완, 수직 아래 = 0도
FOREARM_ANGLE_DEG = 45.483       # 전완, 동일 기준
...
class Body:
    ...
    def __init__(self):
        ...
        # 어깨·팔꿈치를 z축으로 회전시킨다. body_pose 는 관절 1~21 이므로
        #   관절 16(L_shoulder) -> 15,  17(R_shoulder) -> 16
        #   관절 18(L_elbow)    -> 17,  19(R_elbow)    -> 18
        bp = torch.zeros(1, 21 * 3)
        sh = np.deg2rad(SHOULDER_DOWN_DEG)
        el = np.deg2rad(ELBOW_STRAIGHTEN_DEG)
        bp[0, 15 * 3 + 2] = -sh
        bp[0, 16 * 3 + 2] = +sh
        bp[0, 17 * 3 + 2] = -el
        bp[0, 18 * 3 + 2] = +el
...
def fit(photo, height_cm, weight_kg, use_shoe_fix=True, verbose=True, body=None,
    ...
    if save_obj:
        ...
        _save_obj(config.BODY_OBJ, v_scaled, body.faces)
